...
/**
 * Three Way QuickSort
 * running time from linearithmic to linear in a broad class of applications
 * this sort is also entropy-optimal
 * Time Complexity still O(n lg n), but linear when only a constant number of distinct keys
 * @param a {number[]} input array
 * @param low {number} left index
 * @param high {number} right index
 * note there is now a faster algorithm which uses a dual pivot: http://bit.ly/2djgaTn
 * TODO: fix - current implementation will exceed maximum call stack size
 */
export function threeWayQuickSort(a, low, high) {
  if (high <= low) { return; }
  let lt = low;
  let gt = high;
  ...
  let i = low;
  while (i <= gt) {
    if      (a[i] < pivotValue) { exchange(a, lt++, i++); }
    else if (a[i] > pivotValue) { exchange(a, i, gt--); }
    else                          i++; // equal to our pivotValue
  }
...
function merge(a, aux, low, mid, high) {
  ...
  for (let k = low; k <= high; k++) {
    if      (i > mid)         { a[k] = aux[j++]; } // if i is exhausted use next jth element (note a[k] is assigned to aux[j] before j is incremented)
    else if (j > high)        { a[k] = aux[i++]; } // if j is exhausted use next ith element
 ...
 * Time Complexity: O(n) with worst case of quadratic time
 * @param a {number[]}
 * @param k {number}
 * @param less {compareFunction} [compareFunction=lessThan] compares to values of the array
 * to determine if they the first element is less than the second
